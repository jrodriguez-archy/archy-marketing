...
Paper's PDF export is not print-ready. This skill fixes page size, drops the extra page, converts to CMYK and makes a QR pure K. Work in `${CLAUDE_PLUGIN_DATA}/print-pdf/<project>/` (call it `$WORK`). No bundled script is involved; everything below is Ghostscript and qpdf. Create it first (`mkdir -p "$WORK"`) and keep every intermediate file there, never inside `${CLAUDE_PLUGIN_ROOT}`.
