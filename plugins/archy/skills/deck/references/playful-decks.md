...
Body copy runs **24–26px**, not the template's 22: grey descriptive lines under titles read too small at 22. Values and bullet lists at 26/38–42, benefit and policy cells at 24/32–34. When a slide reads heavy, lighten the hierarchy instead of cutting copy (column titles from two lines at 40 to one line at 32). Two sentences stacked as separate nodes read as a broken paragraph: merge them. A wrapping bullet gets a manual break with a hanging indent so its second line clears the bullet.

## Exporting

Emoji go into the `.pptx` as PNG images, not as text (see `archy-design:slides-export`).
