...
Never write reads or payloads inside `${CLAUDE_PLUGIN_ROOT}`. If `build.js` writes the payload beside itself, move it to `$WORK/payloads/`.
...
```bash
node "$TOOLS/build.js" "$WORK/raw/<slug>.jsx" "$WORK/raw/<slug>.tree" --tokens="$WORK/tokens/<slug>.json"
```
...
1. Once: **Plugins → Development → Import plugin from manifest…**, pick `$TOOLS/plugin/manifest.json`.
