...
1. **Install the plugin.** Ask them to click **+** next to the prompt box, choose **Plugins → Add plugin**, find **archy** and install it. The Paper plugin installs with it.
2. **Check.** Ask them to open Paper Desktop with any file, then start a new session in this folder and say "What file is open in Paper?". If Claude answers with the file name, setup is done.
